...
(function() {
    ...
    var konami = (function() {
        ...
        return {
            ...
            frame: document.getElementById('rickrollCont')
        };
    })();
    //konami
    ...
    var nyan = function() {
        notify('nyan!', 'nyan nyan nyan nyan nyan', 'nyan.png', 'nyan.ogg', function() {
            ...
            konami.dom.style.transformStyle = '';
            konami.dom.classList.add('hidden');
            clearInterval(interval);
        });
        konami.image.src = 'images/konami/nyan.gif';
        konami.dom.classList.remove('hidden');
        konami.dom.style.transform = 'perspective(150px)';
        konami.image.style.transition = 'transform 1s ease-out';
        konami.dom.style.transformStyle = 'preserve-3d';
        var interval = setInterval(function() {
            konami.image.style.transform = 'translate3d( ' + Math.floor(Math.random() * 80 - 40) + '%, ' + Math.floor(Math.random() * 80 - 40) + '%, 0) rotateX(' + Math.floor(Math.random() * 20 - 10) + 'deg) rotateY(' + Math.floor(Math.random() * 20 - 10) + 'deg) rotateZ(' + Math.floor(Math.random() * 10 - 5) + 'deg)';
        }, 1000);
    };
    ...
    var nan = function() {
        ...
        konami.image.style.transition = 'transform 1s ease-out';
        konami.dom.style.transformStyle = 'preserve-3d';
        konami.dom.style.background = 'url(images/konami/BSOD.svg)';
        var interval = setInterval(function() {
            konami.image.style.transform = 'translate3d( ' + Math.floor(Math.random() * 80 - 40) + '%, ' + Math.floor(Math.random() * 80 - 40) + '%, 0) rotateX(' + Math.floor(Math.random() * 20 - 10) + 'deg) rotateY(' + Math.floor(Math.random() * 20 - 10) + 'deg) rotateZ(' + Math.floor(Math.random() * 10 - 5) + 'deg)';
        }, 1000);
    };
    ...
    cheet('N A N', {done: nan});
    //tilt
    cheet('t i l t', {done: function() {
        document.body.classList.toggle('tilted');
    }});
    //Rickroll
    document.getElementById('rickroll').addEventListener('click', function(e) {
        e.preventDefault();
        konami.dom.classList.remove('hidden');
        konami.dom.style.background = 'url(images/konami/rickroll.gif)';
        konami.frame.innerHTML = '<iframe width="420" height="345" src="http://www.youtube.com/embed/dQw4w9WgXcQ?autoplay=1" frameborder="0" allowfullscreen></iframe>';
        konami.dom.addEventListener('click', function() {
            konami.frame.innerHTML = '';
            konami.dom.style.background = '';
            konami.dom.removeEventListener('click', this);
        });
    });
    //Minitel
    var minitel = function() {
        document.body.classList.add('minitel');
        play('minitel.ogg', function() {
            document.body.classList.remove('minitel');
        });
    };
    cheet('m i n i t e l', {done: minitel});
    cheet('3 6 1 5', {done: minitel});
    cheet('3 6 space 1 5', {done: minitel});
    cheet('kp_3 kp_6 kp_1 kp_5', {done: minitel});
    cheet('kp_3 kp_6 space kp_1 kp_5', {done: minitel});
})();
